 ...
 * @param {string} newAdminMessage - Новый пароль пользователя (уже хешированный).
 * @param {Function} callback - Функция обратного вызова, принимающая ошибку и результат выполнения.
 */
function changeUserAdminMessage(tableName, userId, newAdminMessage, callback) {
  ...
  const sql = `UPDATE ${tableName} SET adminMessage = ? WHERE id = ?`;

  db.run(sql, [newAdminMessage, userId], function (err) {
    if (err) {
...
module.exports = changeUserAdminMessage;
