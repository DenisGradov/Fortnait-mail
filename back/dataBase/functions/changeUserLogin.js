 ...
 */
function changeUserLogin(tableName, userId, newLogin, callback) {
  ...
  const sql = `UPDATE ${tableName} SET login = ? WHERE id = ?`;

  db.run(sql, [newLogin, userId], function (err) {
    if (err) {
...
module.exports = changeUserLogin;
