 ...
 * @param {string} newLogin - Новый пароль пользователя (уже хешированный).
 * @param {Function} callback - Функция обратного вызова, принимающая ошибку и результат выполнения.
 */
function changeUserBlocking(tableName, userId, blockedStatus, callback) {
  ...
  const sql = `UPDATE ${tableName} SET blocked = ? WHERE id = ?`;

  db.run(sql, [blockedStatus, userId], function (err) {
    if (err) {
...
module.exports = changeUserBlocking;
